...
-- No direct insert/update/delete from clients; use RPCs above.
...
-- Tighten message visibility to members only
...
-- Reactions visibility to members only
...
-- Update dashboard RPC to use membership rather than "posted before"
...
-- Realtime: broadcast join request changes
do $$
begin
  if not exists (
    select 1
